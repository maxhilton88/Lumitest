            ...
            <div className="bg-[#2a1a3e]/60 rounded-2xl border border-amber-500/10 p-4 flex items-center justify-between">
              ...
              <div className="text-right">
                {isPaid ? (
                  ...
                  <span className="text-amber-300/50 text-xs">{todayWatchCount}/1 today</span>
                )}
              </div>
            </div>
            ...
            {videos.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {videos.map((video) => {
                  ...
                  const thumbnail = video.thumbnail_url || (ytId ? `https://img.youtube.com/vi/${ytId}/hqdefault.jpg` : '');
                  
                  return (
                    <button
                      key={video.id}
                      onClick={() => handleWatchVideo(video.youtube_url)}
                      className="bg-[#2a1a3e]/40 rounded-xl border border-amber-500/10 overflow-hidden hover:border-amber-500/30 transition-all group text-left"
                    >
