...
export function useOAuthCallback({
  ...
  useEffect(() => {
    ...
    // Also skip /kg-signup where ?code= is a KG claim code, not OAuth.
    const skipPaths = ['/reset-password', '/kg-signup'];
    if (skipPaths.includes(window.location.pathname)) {
      console.log('[APP] OAuth hook skipping — on', window.location.pathname);
      return;
    }
        ...
        if (result.parent) {
          ...
          if (pendingAssessment?.snapshot) {
            try {
              ...
              // ── Flush pending mastery answers (Bug 3 fix: ChildFlow now stashes these) ──
              if (pendingAssessment.masteryBatch?.length > 0) {
                recordMasteryAnswers(pendingAssessment.masteryBatch).catch((err: any) =>
                  console.warn('[APP] recordMasteryAnswers after OAuth failed (non-blocking):', err)
                );
                console.log(`[APP] Flushed ${pendingAssessment.masteryBatch.length} mastery answers from pre-signup assessment`);
              }
