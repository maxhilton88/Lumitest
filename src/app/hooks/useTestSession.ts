...
export function useTestSession({
  ...
  const [excludedSubjects, setExcludedSubjects] = useState<string[]>(() => {
    try {
      const stored = localStorage.getItem('excluded_subjects');
      if (stored !== null) return JSON.parse(stored);
      const pd = getStoredParentData();
      if (pd?.excluded_subjects && Array.isArray(pd.excluded_subjects)) return pd.excluded_subjects;
      // Backward compat: old boolean → new array
      if (pd?.include_mandarin_test === false) return ['ZH'];
    } catch {}
    return [];
  });
  ...
  const excludedSubjectsRef = useRef<string[]>([]);
  ...
  useEffect(() => {
    ...
    const loadLiveQuests = async () => {
      ...
      try {
        ...
        // Reverse alias map: given a quest subject, what arena IDs should also point to its image?
        const REVERSE_ALIASES: Record<string, string[]> = {
          math: ['numbers'], maths: ['numbers'], mathematics: ['numbers'], matematik: ['numbers'], nombor: ['numbers'], numeracy: ['numbers'], number: ['numbers'],
          chinese: ['mandarin'], cina: ['mandarin'], mandarine: ['mandarin'], '华语': ['mandarin'], '华文': ['mandarin'], '中文': ['mandarin'],
          bm: ['bahasa'], malay: ['bahasa'], melayu: ['bahasa'],
          eng: ['english'], inggeris: ['english'], language: ['english'], literacy: ['english'],
          sains: ['science'], discovery: ['science'], stem: ['science'],
        };
        for (const q of quests) {
          if (q.signed_image_url) {
            // Key by BOTH quest id and subject (lowercase-normalized) so lookups
            // work from QuestSelector (uses quest UUID) AND Training Arena (uses
            // lowercase subject names like "english", "numbers", etc.)
            imageUrls[q.id] = q.signed_image_url;
            if (q.subject) {
              imageUrls[q.subject] = q.signed_image_url;
              const lower = q.subject.toLowerCase();
              imageUrls[lower] = q.signed_image_url;
              // Also key by first word lowercase (e.g. "Bahasa Melayu" → "bahasa")
              const firstWord = q.subject.trim().split(/\s+/)[0].toLowerCase();
              if (firstWord && firstWord !== lower) {
                imageUrls[firstWord] = q.signed_image_url;
              }
              // Reverse alias: e.g. quest subject "Mathematics" → also key as "numbers"
              const aliasTargets = REVERSE_ALIASES[lower] || REVERSE_ALIASES[firstWord] || [];
              for (const alias of aliasTargets) {
                imageUrls[alias] = q.signed_image_url;
              }
            }
          }
        }
        setQuestCardImageUrls(imageUrls);
        console.log(`[QUESTS] Built ${Object.keys(imageUrls).length} quest card image URLs (R2)`);
      } catch (error) {
  ...
  const transformBankQuestions = (bankQuestions: any[], questId: string): Question[] => {
    return bankQuestions.map((bq: any) => {
      ...
      } as Question & { ageDifficulty?: number; quest?: string; dskpCode?: string; bankSubject?: string };
    });
  };
  ...
  const startFreshSession = async (
    ...
    whatsapp: string,
    selectedAge: number,
    selectedExcludedSubjects: string[],
    schoolId: string
  ) => {
    setCompletedModules([]);
    ...
    try {
      const result = await submitLead({
        ...
        includeMandarin: !selectedExcludedSubjects.includes('ZH'),
        score: 0,
        totalQuestions: 0,
        answers: [],
        questResults: [],
  ...
  const handleLanguageStart = async (
    ...
    setAge(selectedAge);
    setExcludedSubjects(selectedExcludedSubjects);
    leadDataRef.current = { childName, parentName, whatsapp };
    ageRef.current = selectedAge;
    excludedSubjectsRef.current = selectedExcludedSubjects;
    ...
    startFreshSession(childName, parentName, whatsapp, selectedAge, selectedExcludedSubjects, schoolId);
  };
  ...
  const handleResumeSession = () => {
    ...
    // Restore excluded subjects from lead data (backward compat with old boolean)
    const restoredExcluded = lead.excluded_subjects || (lead.include_mandarin_test === false ? ['ZH'] : []);
    setExcludedSubjects(restoredExcluded);
    excludedSubjectsRef.current = restoredExcluded;
  ...
  const handleAnswer = (answerId: string) => {
    ...
    const detailedAnswer: DetailedAnswer = {
      ...
      // Capture KSSR taxonomy from question (populated by toQuestionScreenFormat or bank questions)
      skillCode: (currentQuestion as any)._taxonomy?.skillCode || (currentQuestion as any).dskp_code || '',
      kssrLevel: (currentQuestion as any)._taxonomy?.age?.toString() || (currentQuestion as any)._taxonomy?.level || (currentQuestion as any).kssr_level || '',
      topic: (currentQuestion as any)._taxonomy?.topic || (currentQuestion as any).topic || '',
      skillName: (currentQuestion as any)._taxonomy?.subtopic || (currentQuestion as any).skill_name || '',
    };
  ...
  const handleNext = () => {
    if (currentQuestionIndex < currentTestQuestions.length - 1) {
      ...
      // ── Fire mastery log for this module's answers (fire-and-forget) ──
      // Only sends if user is authenticated (has parent_id)
      const parentId = localStorage.getItem('parent_id');
      if (parentId) {
        const subjectDef = SUBJECT_BY_QUEST_ID[currentModule];
        const subjectId = subjectDef?.questId || currentModule;
        const masteryBatch = currentModuleAnswers.map((a) => ({
          subjectId,
          skillCode: a.skillCode || `${subjectId}-${a.questionId}`,
          topicName: a.topic || subjectDef?.name?.en || currentModule,
        ...
        recordMasteryAnswers(masteryBatch).catch((err) => {
          console.error('[TEST SESSION] Mastery log failed (non-blocking):', err);
        });
      }
      ...
      const totalQuestsRequired =
        liveQuestsRef.current.length > 0
          ? liveQuestsRef.current.filter((q) => !q.is_mandarin || excludedSubjectsRef.current.length === 0).length
          : excludedSubjectsRef.current.length === 0
            ? 5
            : 4;
        ...
        if (currentLeadIdRef.current) {
          ...
          try {
            const result = await submitLead({
              ...
              includeMandarin: excludedSubjectsRef.current.length === 0,
              score: reportData.score,
  ...
  const persistAssessmentSnapshot = async (answersToUse?: DetailedAnswer[]) => {
    ...
    try {
      ...
      // NOTE: Per-answer mastery data is now sent per-module in handleNext()
      // to avoid double-counting and to capture partial assessments.
    } catch (error) {
