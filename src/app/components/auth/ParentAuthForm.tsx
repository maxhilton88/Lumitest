...
import { Mail, Lock, User, Eye, EyeOff, Gift, ArrowLeft } from 'lucide-react';
...
export const ParentAuthForm: React.FC<ParentAuthFormProps> = ({
  ...
  // Check URL param for initial mode (e.g. /login?mode=signup from QR flow)
  const initialMode = (() => {
    try {
      const params = new URLSearchParams(window.location.search);
      return params.get('mode') === 'signup' ? 'signup' as const : 'login' as const;
    } catch { return 'login' as const; }
  })();
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      console.log('[PARENT-AUTH] Requesting password reset for:', resetEmail);
      const { error } = await parentAuthClient.auth.resetPasswordForEmail(resetEmail, {
        redirectTo: `${window.location.origin}/reset-password?source=parent`,
      });
      if (error) {
        console.error('[PARENT-AUTH] Password reset error:', error);
        toast.error(`Failed to send reset link: ${error.message}`);
        return;
      }
      console.log('[PARENT-AUTH] Password reset email sent to:', resetEmail);
      toast.success('Reset link sent! Check your email.');
      setResetSent(true);
    } catch (err: any) {
      console.error('[PARENT-AUTH] Password reset exception:', err);
    ...
    <div className="h-[100dvh] relative overflow-hidden flex items-center justify-center p-4 md:p-8">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-4 md:mb-6">
          ...
          >
            {mode === 'forgotPassword'
              ? "We'll send a magic scroll to your email"
              : mode === 'login'
        ...
        <FantasyPanel className="p-6 md:p-8">
          ...
          {mode === 'forgotPassword' ? (
            <>
              {/* Back to login */}
              <button
                onClick={() => { playMenuSelect(); setMode('login'); setResetSent(false); setResetEmail(''); }}
                className="flex items-center gap-2 text-xs font-bold mb-5 transition-colors"
                style={{ color: `${GOLD}90` }}
              >
              ...
              {!resetSent ? (
                <form onSubmit={handleForgotPassword} className="space-y-4">
                  ...
                  <p className="text-[11px] leading-relaxed" style={{ color: `${PARCHMENT}70` }}>
                ...
                <div className="text-center py-4">
                  ...
                  <p className="text-sm mb-5" style={{ color: `${PARCHMENT}80` }}>
                    Check <strong style={{ color: GOLD_LIGHT }}>{resetEmail}</strong> for your password reset link.
                  </p>
                  <button
                    onClick={() => { playMenuSelect(); setMode('login'); setResetSent(false); setResetEmail(''); }}
                    className="w-full py-3 px-4 rounded-xl text-sm font-bold tracking-wider transition-all hover:scale-[1.02] active:scale-[0.98]"
                    style={{
        ...
        {/* Footer links (Store, Privacy, Terms — visible without auth) */}
        <div className="flex items-center justify-center gap-2 flex-wrap mt-4">
          <a href="/store" className="text-[10px] md:text-xs hover:underline transition-colors" style={{ color: `${GOLD}50` }}>
            Pricing
          </a>
          <span className="text-[10px]" style={{ color: `${GOLD}25` }}>&middot;</span>
          <a href="/privacy" className="text-[10px] md:text-xs hover:underline transition-colors" style={{ color: `${GOLD}50` }}>
            Privacy Policy
          </a>
          <span className="text-[10px]" style={{ color: `${GOLD}25` }}>&middot;</span>
          <a href="/terms" className="text-[10px] md:text-xs hover:underline transition-colors" style={{ color: `${GOLD}50` }}>
            Terms of Service
          </a>
        </div>
      </div>
