...
export const GameDashboard: React.FC<GameDashboardProps> = ({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error('[GameDashboard] Failed to load reward config:', err);
      }
    })();
  }, []);
  ...
  const freePracticeLimit = rewardConfig.activities.practice.freeMaxPerDay;
  ...
  const canPractice = isPaid || freePracticeLimit === -1 || todayPracticeCount < freePracticeLimit;
  ...
  const modes = [
    {
      id: 'flashcards',
      title: t('game.cardModeTitle'),
      subtitle: t('game.cardMode'),
      icon: '\uD83D\uDCD1',
      description: t('game.cardModeDesc'),
      color: '#3498db',
      limit: isPaid ? `\u221E ${t('game.unlimited')}` : t('game.freeBrowse'),
      limitColor: isPaid ? '#7cc643' : GOLD,
      action: handleStartFlashcards,
      actionLabel: t('game.startCards'),
      available: true,
    },
    {
      id: 'practice',
      ...
      color: '#7cc643',
      limit: isPaid ? `\u221E ${t('game.unlimited')}` : `${todayPracticeCount}/${freePracticeLimit === -1 ? '\u221E' : freePracticeLimit} ${t('game.today')}`,
    ...
    },
    {
      ...
      color: '#4a90e2',
      limit: isPaid ? `\u221E ${t('game.unlimited')}` : `${todayTestCount}/${freeTestLimit === -1 ? '\u221E' : freeTestLimit} ${t('game.today')}`,
