...
import {
  ...
  Trophy, TrendingUp, Target, Crown, Award, Zap, Lock, X, ArrowRight, Sparkles, Music, BookOpen,
} from 'lucide-react';
import {
...
import { SkillMasteryReport } from './SkillMasteryReport';
import { calculateSessionMastery } from '../../utils/adaptive-engine';
import { ALL_SAMPLE_QUESTIONS } from '../../data/sample-questions';
import { deriveLevelFromAge } from '../../utils/level-utils';
import { WeaknessSpotlight } from './WeaknessSpotlight';
import { MasteryTrendChart } from './MasteryTrendChart';
import { fetchMasteryProfile, type MasteryProfile } from '../../utils/mastery-api';
import { SUBJECTS } from '../../data/kssr-taxonomy';
...
// ===== STAT CARD (Adventure Stats grid item) =====
const StatCard: React.FC<{
  ...
  today?: number;
  subLabel?: string;
}> = ({ icon, accent, total, label, today, subLabel }) => (
  <FantasyPanel className="p-3 text-center relative overflow-hidden">
    <div className="absolute inset-0 opacity-5"
      style={{ background: `radial-gradient(circle at 50% 20%, ${accent}, transparent 70%)` }} />
    <div className="relative z-10">
      <div
        className="w-10 h-10 mx-auto rounded-lg flex items-center justify-center mb-1.5"
      ...
      <p className="text-[9px] uppercase tracking-wider mt-0.5 leading-tight" style={{ color: `${PARCHMENT}75` }}>
        {label}
      </p>
      <div className="mt-1 space-y-0.5">
        {subLabel && (
          <p className="text-[9px] px-1.5 py-0.5 rounded-full inline-block"
...
const LockedPreview: React.FC<{
  ...
  <FantasyPanel className="p-5 md:p-6 relative overflow-hidden">
    ...
    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center"
      ...
      <h4 className="text-sm font-bold text-center mb-1" style={{ fontFamily: CHERRY, color: GOLD_LIGHT }}>
        {description}
      </h4>
...
const getIntensity = (data: ActivityDay | null): number => {
  ...
  const total = data.tests + data.watches + data.practices
    + (data.videos_watched || 0) + (data.songs_listened || 0)
    + (data.flashcards_completed || 0) + (data.battles || 0);
  if (total === 0) return 0;
  if (total === 1) return 1;
  if (total <= 3) return 2;
  return 3;
...
const DayDetailModal: React.FC<{
  ...
  const { t } = useLanguage();
  const total = data ? data.tests + data.watches + data.practices + (data.videos_watched || 0) + (data.songs_listened || 0) + (data.flashcards_completed || 0) + (data.battles || 0) : 0;
          ...
          {total > 0 ? (
            <>
              ...
              <div className="space-y-2.5">
                ...
                {(data!.videos_watched || 0) > 0 && (
                  <div
                    className="flex items-center justify-between px-3.5 py-2.5 rounded-lg"
                    style={{ background: `${GOLD}08`, border: `1px solid ${GOLD}12` }}
                  >
                    <div className="flex items-center gap-2.5">
                      <Eye className="w-4 h-4" style={{ color: '#9b59b6' }} />
                      <span className="text-sm" style={{ color: `${PARCHMENT}90` }}>
                        Videos Watched
                      </span>
                    </div>
                ...
                {(data!.songs_listened || 0) > 0 && (
                  <div
                    className="flex items-center justify-between px-3.5 py-2.5 rounded-lg"
                    style={{ background: `${GOLD}08`, border: `1px solid ${GOLD}12` }}
                  >
                    <div className="flex items-center gap-2.5">
                      <Music className="w-4 h-4" style={{ color: '#f39c12' }} />
                      <span className="text-sm" style={{ color: `${PARCHMENT}90` }}>
                        Songs Listened
                      </span>
                    </div>
                ...
                {(data!.flashcards_completed || 0) > 0 && (
                  <div
                    className="flex items-center justify-between px-3.5 py-2.5 rounded-lg"
                    style={{ background: `${GOLD}08`, border: `1px solid ${GOLD}12` }}
                  >
                    <div className="flex items-center gap-2.5">
                      <BookOpen className="w-4 h-4" style={{ color: '#3498db' }} />
                      <span className="text-sm" style={{ color: `${PARCHMENT}90` }}>
                        {t('mastery.flashcards')}
                      </span>
                    </div>
                    <span className="text-sm font-bold" style={{ color: GOLD_LIGHT }}>
                      {data!.flashcards_completed}
                    </span>
                  </div>
                )}
                {(data!.battles || 0) > 0 && (
  ...
  return (
    <FantasyPanel className="p-4 min-w-[200px] max-w-[220px] flex-shrink-0">
      <div className="text-center mb-3">
        ...
        <h4 className="text-xs font-bold mt-1.5 truncate" style={{ color: GOLD_LIGHT, fontFamily: CHERRY }}>
          {questName}
        </h4>
      </div>
...
const SubjectComparisonChart: React.FC<{ breakdowns: SubjectAgeBreakdown[] }> = ({ breakdowns }) => {
  ...
  // Ensure unique names for recharts internal keying — use index suffix as tiebreaker
  const seen = new Map<string, number>();
  const data = breakdowns.map((b, idx) => {
    const rawName = b.questName || `Subject ${idx + 1}`;
    const raw = rawName.length > 10 ? rawName.slice(0, 9) + '..' : rawName;
    const count = (seen.get(raw) || 0) + 1;
    seen.set(raw, count);
    const name = count > 1 ? `${raw} (${count})` : raw;
    return { name, fullName: rawName, score: b.overallPercentage, fill: CHART_COLORS[idx % CHART_COLORS.length] };
  });
        ...
        <div style={{ width: '55%', height: '100%' }}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              >
                {data.map((entry, index) => (
                  <Cell key={`age-pie-${entry.name}-${index}`} fill={entry.fill} fillOpacity={0.85} />
                ))}
              </Pie>
...
const AccuracyByAgeChart: React.FC<{ breakdowns: SubjectAgeBreakdown[] }> = ({ breakdowns }) => {
  ...
  // Deduplicate short names to avoid recharts key collisions — count-based tiebreaker
  const shortNameMap = new Map<string, string>();
  const shortNameSeen = new Map<string, number>();
  breakdowns.forEach((b) => {
    const raw = b.questName.length > 8 ? b.questName.slice(0, 7) + '..' : b.questName;
    const count = (shortNameSeen.get(raw) || 0) + 1;
    shortNameSeen.set(raw, count);
    const shortName = count > 1 ? `${raw} ${count}` : raw;
    shortNameMap.set(b.questId, shortName);
  });
  ...
  const data = ages.map(age => {
    ...
    breakdowns.forEach((b) => {
      ...
      const pct = ageLevel && ageLevel.total > 0
        ...
        : 0;
      row[shortNameMap.get(b.questId) || b.questName] = pct;
    });
    return row;
  });
...
const ProgressOverTimeChart: React.FC<{ history: AssessmentSnapshot[] }> = ({ history }) => {
  ...
  // Ensure unique date labels for recharts internal keying — count-based tiebreaker
  const dateSeen = new Map<string, number>();
  const data = history.map((h, idx) => {
    const dateStr = h.date ? new Date(h.date).toLocaleDateString('en-MY', { day: '2-digit', month: 'short' }) : `Session ${idx + 1}`;
    const raw = dateStr || `Session ${idx + 1}`;
    const count = (dateSeen.get(raw) || 0) + 1;
    dateSeen.set(raw, count);
    const name = count > 1 ? `${raw} #${count}` : raw;
    return { name, score: h.overallPct, readiness: h.readinessPct, stars: h.totalStars, tp: h.tpLevel };
  });
...
export const MasteryDashboard: React.FC<MasteryDashboardProps> = ({
  ...
  const { t } = useLanguage();
  const practiceNavigate = useNavigate();
  ...
  // Navigate to practice arena pre-filtered by subject
  const handlePracticeSubject = useCallback((subjectId: string) => {
    practiceNavigate(`/realm/practice?subject=${encodeURIComponent(subjectId)}`);
  }, [practiceNavigate]);
  ...
  useEffect(() => {
    ...
    fetchMasteryProfile()
      .then((p) => { setMasteryProfile(p); setMasteryLoading(false); })
      .catch((e) => { console.error('[MASTERY] Profile load failed:', e); setMasteryLoading(false); });
  }, []);
  ...
  if (liveQuests && liveQuests.length > 0) {
    ...
    Object.assign(questNameMap, {
      ...
      sejarah: { name: 'Chronicle Ruins', icon: '\u{1F4DC}' },
      geography: { name: 'Atlas Peaks', icon: '\u{1F30F}' },
    });
  }
  ...
  const snapshotSubjectBreakdowns: SubjectAgeBreakdown[] = lastSnapshot?.subjectSummary
    ...
    : [];
  // Use buildRadarData for dedup (same as live path) to avoid recharts duplicate key warnings
  const snapshotRadarData = buildRadarData(snapshotSubjectBreakdowns);
  ...
  // Activity stats — aggregate lifetime totals from activity timeline (includes realm cross-writes)
  // Fall back to parentData fields for legacy data that predates the cross-write bridge
  const lifetimeFromTimeline = useMemo(() => {
    const totals = { tests: 0, watches: 0, practices: 0, videos_watched: 0, songs_listened: 0, flashcards_completed: 0, battles: 0, questions_total: 0 };
    for (const day of activityData) {
      totals.tests += day.tests || 0;
      totals.watches += day.watches || 0;
      totals.practices += day.practices || 0;
      totals.videos_watched += day.videos_watched || 0;
      totals.songs_listened += day.songs_listened || 0;
      totals.flashcards_completed += day.flashcards_completed || 0;
      totals.battles += day.battles || 0;
      totals.questions_total += day.questions_total || 0;
    }
    return totals;
  }, [activityData]);

  const totalTests = Math.max(parentData?.total_tests || 0, lifetimeFromTimeline.tests);
  const totalWatches = Math.max(parentData?.total_watches || 0, lifetimeFromTimeline.watches);
  const totalPractices = Math.max(parentData?.total_practices || 0, lifetimeFromTimeline.practices);
  const totalPracticeQuestions = Math.max(parentData?.total_practice_questions || 0, lifetimeFromTimeline.questions_total);
  const totalVideos = Math.max(lifetimeFromTimeline.videos_watched, totalWatches);
  const totalMusic = lifetimeFromTimeline.songs_listened;
  const totalFlashcards = lifetimeFromTimeline.flashcards_completed;
  const totalBattles = lifetimeFromTimeline.battles;

  // Today's counts from the latest activity record
  const todayStr = new Date().toISOString().slice(0, 10);
  const todayActivity = activityData.find(d => d.date === todayStr);
  const todayTests = todayActivity?.tests || parentData?.test_count_today || 0;
  const todayPractices = todayActivity?.practices || parentData?.practice_count_today || 0;
  const todayVideos = todayActivity?.videos_watched || 0;
  const todayMusic = todayActivity?.songs_listened || 0;
  const todayFlashcards = todayActivity?.flashcards_completed || 0;
  const todayBattles = todayActivity?.battles || 0;

  // ===== KSSR SKILL MASTERY (from KV profile or adaptive engine fallback) =====
  const derivedChildLevel = useMemo(() => deriveLevelFromAge(childAge), [childAge]);

  // Prefer cumulative KV mastery data; fall back to session-only if unavailable
  const skillMasteryData = useMemo(() => {
    // Try KV mastery profile first (cumulative across all sessions)
    if (masteryProfile && masteryProfile.subjects.length > 0) {
      return masteryProfile.subjects.map((subj): SubjectMastery => {
        // Map questId → SubjectCode
        const subjDef = SUBJECTS.find(s => s.questId === subj.subjectId);
        const code = subjDef?.code || subj.subjectId.toUpperCase().slice(0, 3);
        return {
          subject: code,
          topicMastery: (subj.topics || []).map(t => ({
            topic: t.topicName,
          ...
          overallCorrect: subj.totalCorrect,
          overallTotal: subj.totalAttempts,
          overallPct: subj.percentage,
          functionalLevel: derivedChildLevel.level,
        };
      });
    }
    // Fallback: build from current session's allAnswers via adaptive engine
    if (!allAnswers || allAnswers.length === 0) return [];
    const levelHistory = allAnswers.map(a => ({
      level: derivedChildLevel.level,
      questionId: a.questionId,
      correct: a.isCorrect,
    }));
    return calculateSessionMastery(levelHistory, ALL_SAMPLE_QUESTIONS as any);
  }, [allAnswers, derivedChildLevel, masteryProfile]);
  ...
  return (
    <div className="space-y-6">
        ...
        <FantasyPanel className="p-5 md:p-8" gold>
          <div className="text-center mb-3">
            <div className="inline-flex items-center gap-2 mb-1">
              ...
              <h2 className="text-lg md:text-xl font-bold" style={{ fontFamily: CHERRY, color: GOLD_LIGHT }}>
      ...
      {/* ═══════════════════════════════════════════
          SECTION 3.5: WEAKNESS SPOTLIGHT — PER-SUBJECT WEAK SKILLS
         ═══════════════════════════════════════════ */}
      <WeaknessSpotlight
        masteryProfile={masteryProfile}
        loading={masteryLoading}
        onPractice={handlePracticeSubject}
      />

      {/* ═══════════════════════════════════════════
          SECTION 3.6: MASTERY TREND — SKILL PROGRESS OVER TIME
         ═══════════════════════════════════════════ */}
      <MasteryTrendChart visible={!masteryLoading} />
      ...
      >
        ...
        <div className="relative z-10 p-4 md:p-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-base md:text-lg font-black" style={{ fontFamily: CHERRY, color: GOLD_LIGHT }}>
              {childName || 'Explorer'}
            </h2>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {/* Quests */}
          <StatCard
            icon={<Swords className="w-6 h-6" style={{ color: '#ff6b35' }} />}
            accent="#ff6b35"
            total={totalTests}
            label={t('mastery.totalQuests')}
            today={todayTests}
          />
          {/* Videos */}
          <StatCard
            icon={<Eye className="w-6 h-6" style={{ color: '#9b59b6' }} />}
            accent="#9b59b6"
            total={totalVideos}
            label={t('mastery.totalVideos')}
            today={todayVideos}
          />
          {/* Practices */}
          <StatCard
            icon={<Shield className="w-6 h-6" style={{ color: '#27ae60' }} />}
            accent="#27ae60"
            total={totalPractices}
            label={t('mastery.totalTraining')}
            today={todayPractices}
            subLabel={totalPracticeQuestions > 0 ? `${totalPracticeQuestions} ${t('mastery.questionsAnswered')}` : undefined}
          />
          {/* Music */}
          <StatCard
            icon={<Music className="w-6 h-6" style={{ color: '#3498db' }} />}
            accent="#3498db"
            total={totalMusic}
            label={t('mastery.totalMusic') || 'Music'}
            today={todayMusic}
          />
          {/* Flashcards */}
          <StatCard
            icon={<BookOpen className="w-6 h-6" style={{ color: '#e67e22' }} />}
            accent="#e67e22"
            total={totalFlashcards}
            label={t('mastery.totalFlashcards') || 'Flashcards'}
            today={todayFlashcards}
          />
          {/* Battles */}
          <StatCard
            icon={<Flame className="w-6 h-6" style={{ color: '#e74c3c' }} />}
            accent="#e74c3c"
            total={totalBattles}
            label={t('mastery.totalBattles') || 'Battles'}
            today={todayBattles}
          />
        </div>
      </div>
      ...
      {(hasData || hasSnapshot) && subjectBreakdowns.length > 0 ? (
        <div className="space-y-4">
          <div className="text-center">
            <h2 className="text-lg font-bold" style={{ fontFamily: CHERRY, color: GOLD_LIGHT }}>
        ...
        <div className="space-y-4">
          <div className="text-center">
            <h2 className="text-lg font-bold" style={{ fontFamily: CHERRY, color: GOLD_LIGHT, opacity: 0.4 }}>
              {t('mastery.subjectComparison')}
            </h2>
            <GoldOrnament className="mt-2" />
          </div>
      ...
      {/* ═══════════════════════════════════════════
          SECTION 10: KSSR SKILL MASTERY (NEW TAXONOMY)
         ═══════════════════════════════════════════ */}
      <FantasyPanel className="p-5 md:p-6">
        <SectionHeader
          icon={<BookOpen className="w-5 h-5" style={{ color: '#9b59b6' }} />}
          title={t('mastery.skillMasteryTitle') || 'Skill Mastery'}
          subtitle={t('mastery.skillMasterySub') || 'KSSR skill-level breakdown by subject & topic'}
        />
        <SkillMasteryReport
          masteryData={skillMasteryData}
          childName={childName}
          childLevel={derivedChildLevel?.level}
        />
      </FantasyPanel>
    </div>
  );
