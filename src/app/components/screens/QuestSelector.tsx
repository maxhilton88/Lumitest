...
import { Lock, Play, Star, ChevronLeft, ChevronRight, ArrowLeft, Swords } from 'lucide-react';
...
// Default card images per module
const DEFAULT_CARD_IMAGES: Record<string, string> = {
  ...
  sejarah: 'https://images.unsplash.com/photo-1767533427305-7a1f61c92fb3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=600',
  geography: 'https://images.unsplash.com/photo-1723306743301-5690e685fd22?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=600',
};
...
interface SubjectModule {
  ...
  color: string;
  conditional?: boolean;
  subject?: string; // lowercase subject key for image lookups
}

const ALL_MODULES: SubjectModule[] = [
  {
    id: 'english',
  ...
  {
    id: 'sejarah',
    name: { en: 'Chronicle Ruins', ms: 'Runtuhan Kronik', zh: '编年史遗迹' },
    subtitle: { en: 'History (Sejarah)', ms: 'Sejarah', zh: '历史' },
    icon: '📜',
    color: '#d97706',
  },
  {
    id: 'geography',
    name: { en: 'Atlas Peaks', ms: 'Puncak Atlas', zh: '地图之巅' },
    subtitle: { en: 'Geography', ms: 'Geografi', zh: '地理' },
    icon: '🌏',
    color: '#059669',
  },
];
...
export const ModuleSelector: React.FC<ModuleSelectorProps> = ({
  ...
  const modules: SubjectModule[] = React.useMemo(() => {
    if (liveQuests && liveQuests.length > 0) {
      return liveQuests
        .filter(q => {
          // Filter by excludedSubjects using SUBJECT_BY_QUEST_ID lookup
          if (excludedSubjects.length === 0) return true;
          // Check is_mandarin for backward compat
          if (q.is_mandarin && excludedSubjects.includes('ZH')) return false;
          return true;
        })
        .map(q => ({
          id: q.id,
    ...
    return ALL_MODULES.filter(
      (m) => !m.conditional || !excludedSubjects.includes('ZH')
    );
  }, [liveQuests, excludedSubjects]);
  ...
  const getCardImage = (moduleId: string, subject?: string) => {
    // Try by quest UUID first (for uploaded images), then by subject name (for defaults)
    const subjectKey = subject || moduleId;
    return (
      brandingSettings.questCardImages?.[moduleId] ||
      brandingSettings.questCardImages?.[subjectKey] ||
      DEFAULT_CARD_IMAGES[subjectKey] ||
  ...
  // ── Card tap handler ──
  // Test mode: cards are NOT clickable (user must use the Start button)
  // Practice mode: all cards are clickable
  const handleCardTap = (module: SubjectModule, index: number) => {
    if (practiceMode) {
      onModuleSelect(module.id);
      return;
    }
    // Test mode — no card tapping allowed; just scroll to center it
    scrollToCard(index);
    setCenteredIndex(index);
  };

  // ── Start Test handler — starts the next active module ──
  const handleStartTest = () => {
    if (allCompleted) return;
    const activeModule = modules[activeIndex];
    if (activeModule) {
      onModuleSelect(activeModule.id);
    }
  };
  ...
  // Active module name for the Start button
  const activeModuleName = !allCompleted && modules[activeIndex]
    ? modules[activeIndex].name[language]
    : '';
    ...
    <div className="h-[100dvh] relative overflow-hidden flex flex-col">
      ...
      {onBack && (
        ...
        >
          ...
          <span
            ...
            style={{ color: '#c8b88a', fontFamily: CHERRY }}
          >
      ...
      <div className="relative z-10 text-center px-4 md:px-8 pt-8 md:pt-12 pb-2">
        ...
        <FantasyTitle size="lg">
          {practiceMode
            ? (language === 'en' ? 'Training Grounds'
              : language === 'ms' ? 'Padang Latihan'
              : '训练场')
            : (language === 'en' ? 'Test Journey'
              : language === 'ms' ? 'Perjalanan Ujian'
              : '测试旅程')}
        </FantasyTitle>
        ...
        >
          {practiceMode
            ? (language === 'en' ? 'Pick Any Subject — No Limits'
              : language === 'ms' ? 'Pilih Subjek — Tiada Had'
              : '选择任意科目 — 无限制')
            : (language === 'en'
        ...
        >
          <div className="flex gap-4 md:gap-6">
            {modules.map((module, index) => {
              ...
              const cardImage = getCardImage(module.id, module.subject);
                ...
                >
                  ...
                  <div
                    ...
                    className={`
                      ...
                      ${practiceMode ? 'cursor-pointer' : 'cursor-default'}
                  ...
                  >
                    ...
                    {/* Module Number Badge — shield/medallion style (Cinzel for small number) */}
                    <div
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5 z-10">
                      ...
                      {isCompleted && result && !practiceMode && (
                        <div className={`mt-3 ${starsAnimating && justCompletedModule === module.id ? 'animate-bounce' : ''}`}>
                          ...
                          <p className="text-xs mt-1" style={{ color: '#c8b88a99', fontFamily: CINZEL }}>
                      ...
                      {/* PRACTICE mode: per-card TRAIN button (active + centered) */}
                      {practiceMode && isCentered && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            onModuleSelect(module.id);
                          ...
                          style={{
                            fontFamily: CHERRY,
                            background: 'linear-gradient(135deg, #7cc643 0%, #5ea832 100%)',
                            color: '#fff',
                            border: '2px solid #a3e66088',
                            boxShadow: '0 4px 20px rgba(124,198,67,0.3), inset 0 1px 0 rgba(255,255,255,0.2)',
                            textShadow: '0 1px 2px rgba(0,0,0,0.3)',
                          }}
                        >
                          <Play className="w-4 h-4 md:w-5 md:h-5" fill="#fff" />
                          {language === 'en' ? 'TRAIN' : language === 'ms' ? 'LATIH' : '训练'}
                        </button>
                      )}

                      {/* TEST mode: show "Up Next" badge on active card */}
                      {!practiceMode && isActive && isCentered && !allCompleted && (
                        <div
                          className="mt-3 px-3 py-1.5 rounded-full inline-flex items-center gap-1.5"
                          style={{
                            background: 'rgba(212,164,74,0.15)',
                            border: '1px solid rgba(212,164,74,0.3)',
                          }}
                        >
                          <Swords className="w-3.5 h-3.5" style={{ color: '#ffd700' }} />
                          <span
                            className="text-xs font-bold"
                            style={{ color: '#ffd700', fontFamily: CHERRY }}
                          >
                      ...
                      {isLocked && !isUnlocking && isCentered && !practiceMode && (
                        <div className="mt-3 text-center">
                          <p className="text-xs md:text-sm italic" style={{ color: '#d4a44a66' }}>
                            {language === 'en'
                              ? `Complete Test ${index} first`
                              : language === 'ms'
                              ? `Selesaikan Ujian ${index} dahulu`
                              : `先完成测试 ${index}`}
                          </p>
                        </div>
                      )}
      ...
      {/* ── START TEST BUTTON (Test mode only, not practice) ── */}
      {!practiceMode && !allCompleted && (
        <div className="relative z-10 flex flex-col items-center px-6 pb-2">
          <button
            onClick={handleStartTest}
            className="w-full max-w-xs py-3.5 md:py-4 rounded-2xl font-bold text-base md:text-lg flex items-center justify-center gap-2.5 transition-all duration-200 active:scale-95 hover:scale-[1.02] tracking-wider uppercase"
            style={{
              fontFamily: CHERRY,
          ...
          <p
            ...
            style={{ color: '#c8b88a88', fontFamily: CHERRY }}
          >
            {activeModuleName}
          </p>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center px-6 pb-2">
          ...
          >
            <p
              ...
              style={{ color: '#ffd700', fontFamily: CHERRY }}
            >
              {language === 'en' ? 'All Tests Complete!' : language === 'ms' ? 'Semua Ujian Selesai!' : '所有测试已完成！'}
            </p>
            <p className="text-xs mt-1" style={{ color: '#c8b88a88' }}>
              {language === 'en' ? 'Great work, hero!' : language === 'ms' ? 'Kerja bagus, wira!' : '做得好，英雄！'}
            </p>
          </div>
        </div>
      )}
