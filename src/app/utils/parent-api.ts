...
export async function parentValidateSession() {
  ...
  if (!response.ok) {
    ...
    try { await parentAuthClient.auth.signOut(); } catch {}
    return null;
  }
...
export function parentLogout() {
  ...
  localStorage.removeItem('excluded_subjects');
  // Clear the Supabase client session too
  try { parentAuthClient.auth.signOut(); } catch {}
}
...
export async function createToyCheckoutSession(email: string) {
  const parentId = localStorage.getItem('parent_id');
  if (!parentId) throw new Error('Not logged in');

  const response = await fetch(`${API_BASE}/stripe/checkout-toy`, {
    method: 'POST',
    headers: await getParentAuthHeaders(),
    body: JSON.stringify({
      parentId,
  ...
  const result = await response.json();
  if (!response.ok) {
    throw new Error(result.error || 'Toy checkout failed');
  }
...
export async function fetchVideos() {
  ...
  if (!response.ok) return { videos: [], series: [], categories: [] };
  const result = await response.json();
  return { videos: result.videos || [], series: result.series || [], categories: result.categories || [] };
}

// ===== DYNTUBE PLAYBACK URL =====

export async function fetchPlaybackUrl(dyntubeKey: string): Promise<{ hlsUrl: string; thumbnail: string; raw?: any }> {
  try {
    const response = await fetch(`${API_BASE}/videos/playback/${dyntubeKey}`, {
      method: 'GET',
      headers: getPublicHeaders(),
    });
    ...
    if (!response.ok) {
      ...
      console.error('[PARENT-API] Playback URL fetch failed:', errText);
      return { hlsUrl: '', thumbnail: '' };
    }

    const result = await response.json();
    console.log('[PARENT-API] Playback URL result:', result);
    return { hlsUrl: result.hlsUrl || '', thumbnail: result.thumbnail || '', raw: result.raw };
  } catch (err) {
    console.error('[PARENT-API] Playback URL error:', err);
    return { hlsUrl: '', thumbnail: '' };
  }
}
...
export async function logPlayEvent(type: 'video' | 'music', videoId?: string, trackId?: string) {
  try {
    const response = await fetch(`${API_BASE}/parent/play-event`, {
      method: 'POST',
      headers: await getParentAuthHeaders(),
      body: JSON.stringify({ type, videoId, trackId }),
    });
    if (!response.ok) {
      console.warn('[PARENT-API] Play event log failed');
    }
  } catch (err) {
    console.warn('[PARENT-API] Play event error:', err);
  }
}

// ===== WATCH HISTORY =====

export async function fetchWatchHistory(): Promise<{ videoId: string; watchedAt: string }[]> {
  try {
    const response = await fetch(`${API_BASE}/parent/watch-history`, {
      method: 'GET',
      headers: await getParentAuthHeaders(),
    });
...
// ===== KG INFO (logo + name) by short code =====

export async function fetchKGInfo(shortCode: string): Promise<{ logoUrl: string; schoolName: string } | null> {
  try {
    const response = await fetch(`${API_BASE}/schools/resolve/${encodeURIComponent(shortCode)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${publicAnonKey}`,
    ...
    const result = await response.json();
    return {
      logoUrl: result.school?.branding?.logoUrl || '',
      schoolName: result.school?.schoolName || '',
    };
  } catch (err) {
    console.warn('[PARENT-API] fetchKGInfo error:', err);
    return null;
  }
}
...
export async function connectToKG(code: string) {
  const response = await fetch(`${API_BASE}/parent/connect-kg`, {
    method: 'POST',
    headers: await getParentAuthHeaders(),
  ...
  // Refresh parent data so kg_connection appears in local state
  if (result.connection) {
    const stored = localStorage.getItem('parent_data');
    if (stored) {
      try {
        const parsed = JSON.parse(stored);
        parsed.kg_connection = result.connection;
        localStorage.setItem('parent_data', JSON.stringify(parsed));
      } catch (_) {}
    }
  }
  return result;
...
export async function disconnectFromKG() {
  const response = await fetch(`${API_BASE}/parent/connect-kg`, {
    method: 'DELETE',
    headers: await getParentAuthHeaders(),
  });
  ...
  // Clear kg_connection from local state
  const stored = localStorage.getItem('parent_data');
  if (stored) {
    try {
      const parsed = JSON.parse(stored);
      delete parsed.kg_connection;
      localStorage.setItem('parent_data', JSON.stringify(parsed));
    } catch (_) {}
  }
  return result;
