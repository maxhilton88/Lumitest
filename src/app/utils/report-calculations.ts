...
export interface DetailedAnswer {
  ...
  /** KSSR taxonomy metadata (optional — populated from _taxonomy on questions) */
  skillCode?: string;
  kssrLevel?: string;
  topic?: string;
  skillName?: string;
...
export function buildRadarData(
  ...
  // Ensure unique subject names for recharts keying
  const nameCounts = new Map<string, number>();
  return breakdowns.map((b, idx) => {
    let subject = b.questName || `Subject ${idx + 1}`;
    const prev = nameCounts.get(subject) || 0;
    nameCounts.set(subject, prev + 1);
    if (prev > 0) subject = `${subject} (${prev + 1})`;
    return { name: subject, subject, functionalAge: b.functionalAge, fullMark: 7 };
  });
}
